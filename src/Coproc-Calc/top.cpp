 ...
 *      Telecom ParisTECH
 *
 *      Testbench for the incremental calculation module
 *
 ************************************************************/
...
int sc_main(int argc, char *argv[])
{
  ...
  /* instanciates the module and generates the traces */
  ...
  /* tests if the not !en signal is working as required, i.e. the output does not change */
  ...
  //for each tile, calculate the output pixel
  for(j=0; j<30; j++) //30 vertically
	...
	while(x_valid && !finished)
	  {
	    /* x_exp is the expected value given the x and y coordinates and the coefficients */  
	    ...
	    /* compares the expected value with the observed value and outputs an error if the error is greater than 3% */
	    if((to_float(mfixed(o_dx,o_x)) - to_float(x_exp))>0.03*to_float(mfixed(o_dx,o_x)))
	      cout << "ERROR" << endl;

	    // x++
	    x = fx_add(x,mfixed(1));
	    if (x == (fx_add(mfixed(TILE_WIDTH),(fx_mul(mfixed(16),mfixed(i)))))) //if x == 16
	      {
		x = fx_mul(mfixed(16),mfixed(i)); // x = 16 * i
		y = fx_add(y,mfixed(1)); // y++
	      }
...
void calc_coeffs (int i, int j)
// function to instantiate the Coprocessor module with the coefficients for incremental calculation
{
